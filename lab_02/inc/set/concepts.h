...
concept ConvertibleRange = !IsSet<R> && Range<R> && Convertible<std::ranges::range_value_t<R>, T> && !Container<R>;
...
template <typename C, typename T>
concept CommonContainer = !IsSet<C> && Container<C> && HasCommon<typename std::remove_reference_t<C>::value_type, T>;
